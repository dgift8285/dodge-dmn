...
export async function startSession(sessionId, { restore = false, pairingNumber = null } = {}) {
  ...
  const entry = { sock: null, qr: null, connected: false, lastActivity: Date.now(), profilePicSet: false, pairingCode: null };
  ...
  // If pairing number provided, request code as soon as socket is ready
  if (pairingNumber && !state.creds.registered) {
    sock.ev.on('connection.update', async (update) => {
      if (update.qr && !entry.pairingCode) {
        try {
